...
import { ActivitiesData } from "@/Data/General/Widgets/General";
import { Badge, Card, CardBody, CardHeader, Col } from "reactstrap";

const Activities = ({ col }: { col?: string }) => {
  return (
    <Col xl={col ? Number(col) : 4} md="6" className="order-md-viii">
            ...
            {ActivitiesData.map((item, i) => (
              <li key={i} className="d-flex align-items-center">
                ...
                <div className="d-flex w-100 align-items-center">
                  <div>{item.h6 ? (<div className="d-flex p-0 m-0"><h6 className="">{item.message}</h6> <h6 className="txt-primary">{item.code}</h6></div>) : <span className="f-w-500 f-12 f-light">{item.message}</span>}</div>
